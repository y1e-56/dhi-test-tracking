...
import { products as seedProducts, type ProductDocumentType } from "@/lib/dhi-data";
import { productTabs } from "@/lib/dhi-nav";
import { useI18n } from "@/lib/i18n";
import { getUser, productVisibleTo } from "@/lib/access";
import { canUploadProductDoc } from "@/lib/document-permissions";
import { ProductAccessDenied } from "@/components/dhi/AccessDenied";
import { uploadEvidence } from "@/lib/api";

export const Route = createFileRoute("/produits/$productId/documents/ajouter")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `Ajouter un document · ${loaderData?.name ?? "Produit"} — DHI Quality Platform` },
    ],
  }),
  component: AddProductDocument,
});
...
function AddProductDocument() {
  ...
  const { products, addProductDocument } = useStore();
  const product = products.find((p) => p.id === productId);

  const [type, setType] = useState<ProductDocumentType>("cdc");
      ...
      <div className="panel p-6 pl-12 sm:p-8 sm:pl-16 xl:pl-20">
        ...
        <form onSubmit={submit} className="max-w-4xl space-y-8">
            ...
            <div className="grid gap-4">
              <div className="grid gap-2">
                ...
                <Input
                  id="d-type"
                  value={type}
                  onChange={(e) => setType(e.target.value as ProductDocumentType)}
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-border">
            ...
            <Button type="submit" className="h-11 px-6" disabled={!canUploadProductDoc(type)}>
